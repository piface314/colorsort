...
function theme.Button(text, opt, x,y,w,h)
	...
	y = y + theme.getVerticalOffsetForAlign(opt.valign, opt.font, h)
	local p = opt.font:getWidth("  ")
	love.graphics.printf(text, x+p, y, w-p*2, opt.align or "center")
end

function theme.IconButton(img, quad, opt, x,y,w,h)
	local c = theme.getColorForState(opt)

	theme.drawBox(x,y,w,h, c, opt.cornerRadius)
	theme.Icon(img, quad, opt, x,y,w,h)
end

function theme.Icon(img, quad, opt, x,y,w,h)
	local c = theme.getColorForState(opt)
	love.graphics.setColor(c.fg)
	local wi, hi, s
	if quad then
		_, _, wi, hi = quad:getViewport()
	else
		wi, hi = img:getDimensions()
	end
	...
	if quad then
		love.graphics.draw(img, quad, x+w/2-wi/2, y+h/2-hi/2, 0, s)
	else
		love.graphics.draw(img, x+w/2-wi/2, y+h/2-hi/2, 0, s)
	end
end
